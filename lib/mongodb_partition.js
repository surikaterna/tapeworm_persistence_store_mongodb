...
function checkNonIndexOptionsConflictError(err) {
  return Boolean(err && err.codeName !== 'IndexOptionsConflict');
}
...
MdbPartition.prototype.open = function () {
  ...
  return new Promise(function (resolve, reject) {
    self._snapshots.ensureIndex({ id: 1 }, { unique: true }, function (err) {
      if (checkNonIndexOptionsConflictError(err)) {
        return reject(err);
      }
      self._commits.ensureIndex({ id: 1 }, { unique: true }, function (err) {
        if (checkNonIndexOptionsConflictError(err)) {
          return reject(err);
        }
        self._commits.ensureIndex({ streamId: 1, commitSequence: 1 }, { unique: true }, function (err) {
          if (checkNonIndexOptionsConflictError(err)) {
            return reject(err);
          }
          self._commits.ensureIndex({ createDateTime: 1 }, { unique: false }, function (err) {
            if (checkNonIndexOptionsConflictError(err)) {
              return reject(err);
            }
            resolve(self);
