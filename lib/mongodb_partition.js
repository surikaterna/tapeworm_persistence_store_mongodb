...
const streamIdIndex = { streamId: 1, commitSequence: 1 };

var MdbPartition = function (mongodb, partitionId, options = {}) {
...
MdbPartition.prototype.open = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    self._commits.createIndex({ id: 1 }, { unique: true }, function (err, indexName) {
      if (err) {
        reject(err);
      } else {
        self._commits.createIndex(streamIdIndex, { unique: true }, function (err, indexName) {
          if (err) {
            reject(err);
          } else {
            self._commits.createIndex({ createDateTime: 1 }, { unique: false }, function (err, indexName) {
              if (err) {
                reject(err);
              } else {
                resolve(self);
...
MdbPartition.prototype.storeSnapshot = function (streamId, snapshot, version, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    self._snapshots.updateOne({ _id: streamId }, { $set: toStore }, { upsert: true }, function (err, res) {
      if (err) {
        reject(err);
      } else {
        resolve(toStore);
...
MdbPartition.prototype.append = function (commit, callback) {
  ...
  return new Promise(function (resolve, reject) {
    self._commits.insertOne(commit, function (err, result) {
      if (err) {
...
MdbPartition.prototype.queryStreamFallback = function (streamId, fromEventSequence, callback) {
  ...
  const eventVersionFilter = !fromEventSequence ? {} : {
    'events.version': {
      $gte: fromEventSequence
    }
  }

  var self = this;
  return new Promise(function (resolve, reject) {
    const query = Object.assign({ streamId }, eventVersionFilter, _.get(self._options, 'fallbackFilter', {}));
    //TODO: sort in insert order...
    self._commits.find(query).hint(streamIdIndex).sort({ commitSequence: 1 }).toArray(function (err, commits) {
      if (err) {
        reject(err);
      } else {
        if (fromEventSequence && commits && commits[0] && commits[0].events) {
          commits[0].events = commits[0].events.filter((event) => (event.version || 0) >= fromEventSequence);
        }
        resolve(commits);
      }
    });
