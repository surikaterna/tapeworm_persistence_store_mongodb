...
MdbPartition.prototype.open = function () {
  ...
  return this._snapshots.createIndex({ id: 1 }, { unique: true })
    .then(() => this._commits.createIndex({ id: 1 }, { unique: true }))
    .then(() => this._commits.createIndex({ streamId: 1, commitSequence: 1 }, { unique: true }))
    .then(() => this._commits.createIndex({ createDateTime: 1 }, { unique: false }))
    .then(() => this);
};
